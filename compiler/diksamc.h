...
#define dkc_is_math_operator(boperator) \
  ((boperator) == ADD_EXPRESSION || (boperator) == SUB_EXPRESSION\
   || (boperator) == MUL_EXPRESSION || (boperator) == DIV_EXPRESSION\
   || (boperator) == MOD_EXPRESSION)

#define dkc_is_compare_operator(boperator) \
  ((boperator) == EQ_EXPRESSION || (boperator) == NE_EXPRESSION\
   || (boperator) == GT_EXPRESSION || (boperator) == GE_EXPRESSION\
   || (boperator) == LT_EXPRESSION || (boperator) == LE_EXPRESSION)
...
#define dkc_is_var(type) \
  ((type)->basic_type == DVM_VARIENT_TYPE && (type)->derive == NULL)
...
#define dkc_is_logical_operator(boperator) \
  ((boperator) == LOGICAL_AND_EXPRESSION || (boperator) == LOGICAL_OR_EXPRESSION)
...
dkc_create_function_definition(TypeSpecifier *type, char *identifier,
                               ParameterList *parameter_list,
                               ExceptionList *throws, Block *block,int isApost);
void dkc_function_define(TypeSpecifier *type, char *identifier,
                         ParameterList *parameter_list,
                         ExceptionList *throws, Block *block,int isApose);
...
Expression *dkc_create_assign_expression(Expression *left,
                                         AssignmentOperator boperator,
                                         Expression *operand);
Expression *dkc_create_binary_expression(ExpressionKind boperator,
                                         Expression *left,
                                         Expression *right);
...
Expression *BcCreateAutoExpression(char *identifier);
...
Expression *dkc_create_new_expression(char *class_name, char *method_name,
                                      ArgumentList *argument);
Expression *dkc_create_new_delegate_expression();
...
void dkc_create_delegate_definition(TypeSpecifier *type, char *identifier,
                                    ParameterList *parameter_list,
                                    ExceptionList *throws,int isApost);
