...
alloc_cast_expression(CastType cast_type, Expression *operand)
{
	...
	else if (cast_type == INT_TO_VAR_CAST
               || cast_type == DOUBLE_TO_VAR_CAST || cast_type == STRING_TO_VAR_CAST )
	{
        cast_expr->type = dkc_alloc_type_specifier(DVM_VARIENT_TYPE);
...
static Expression *
create_var_to_cast(Expression *src,TypeSpecifier* ty)
{
    ...
    if (dkc_is_boolean(ty)) {
        ...
        cast = alloc_cast_expression(VAR_TO_STRING_CAST, src);
    }
...
static Expression *
create_to_var_cast(Expression *src)
{
    Expression *cast = NULL;
...
create_assign_cast(Expression *src, TypeSpecifier *dest)
{
	...
	else if (dkc_is_var(src->type))
	{
		...
		if(dkc_is_string(dest))
			return alloc_cast_expression(VAR_TO_STRING_CAST, src);
		if(dkc_is_double(dest))
	...
	{
        cast_expr = create_to_var_cast(src);
        if (cast_expr) {
            return cast_expr;
        }
	}
	...
	else if (dkc_is_delegate(dest) && dkc_is_delegate(src->type) && src->kind==NEW_EXPRESSION )
	{
		return src;
	}
        ...
        expr->type = dkc_alloc_type_specifier(DVM_DOUBLE_TYPE);
               ...
               && ((dkc_is_string(expr->u.binary_expression.left->type)
                   ...
                   || (dkc_is_string(expr->u.binary_expression.left->type)
                       ...
                       == NULL_EXPRESSION))) {
        expr->type = dkc_alloc_type_specifier(DVM_STRING_TYPE);
	} else if ((dkc_is_var(expr->u.binary_expression.left->type)
		&& dkc_is_var(expr->u.binary_expression.right->type)))
	{
        expr->type = dkc_alloc_type_specifier(DVM_VARIENT_TYPE);
    } else {
        dkc_compile_error(expr->line_number,
    ...
    if (func_expr->kind == IDENTIFIER_EXPRESSION) {
        ...
        if (dkc_is_array(func_expr->u.member_expression.expression->type)) {
            ...
            fd = &compiler->string_method[func_expr->u.member_expression
                                          .method_index];
			//fix-me : here to add built-in methods for varients
        } else {
                      ...
                      ExceptionList **el_p)
{
    ...
    if (dkc_is_class_object(obj->type)) {
		...
		//fix-me : here to add method codes for varients
    } else {
        dkc_compile_error(expr->line_number,
                     ...
                     ExceptionList **el_p)
{
    ...
    if (ie->barray->type->derive != NULL
        ...
        expr->type->derive = ie->barray->type->derive->next;
    } else if (dkc_is_string(ie->barray->type)) {
        expr->type = dkc_alloc_type_specifier(DVM_INT_TYPE);
		//fix-me : here to add index codes for varients
    } else {
        dkc_compile_error(expr->line_number,
    ...
    if (!dkc_is_int(ie->index->type)) {
		...
		{
			ie->index=alloc_cast_expression(VAR_TO_INT_CAST,ie->index);
		}
		else
		{
			dkc_compile_error(expr->line_number,
                       ...
                       ExceptionList **el_p)
{
    ...
    if (!dkc_is_int(expr->u.inc_dec.operand->type) && !dkc_is_var(expr->u.inc_dec.operand->type)) {
        dkc_compile_error(expr->line_number,
                   ...
                   ExceptionList **el_p)
{
	...
	{
		type = dkc_alloc_type_specifier(DVM_DELEGATE_TYPE);
		expr->type = type;
		return expr;
	}
    ...
    if (return_value == NULL) {
        if (fd->type->derive) {
            ...
            case DVM_DELEGATE_TYPE:
                return_value = dkc_alloc_expression(NULL_EXPRESSION); //check-me : why null?
                break;
            case DVM_ENUM_TYPE:
                return_value
...
static void
fix_parameter_index(FunctionDefinition *fd)
{
	int i;
	for(i=0;i<fd->local_variable_count;i++)
	{
		if(fd->local_variable[i]->is_local )
		{
			fd->local_variable[i]->variable_index=fd->local_variable_count-1-fd->local_variable[i]->variable_index;
		}
	}
}
...
fix_delegate_list(DKC_Compiler *compiler)
{
    ...
    for (dd_pos = compiler->delegate_definition_list; dd_pos;
        ...
        dd_pos->param_cnt=fix_parameter_list(dd_pos->parameter_list);
