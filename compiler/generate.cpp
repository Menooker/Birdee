...
alloc_executable(PackageName *package_name)
{
    ...
    exe =(DVM_Executable*) MEM_malloc(sizeof(DVM_Executable));
...
int add_constant_pool(DVM_Executable *exe, DVM_ConstantPool *cp)
{
    int ret;

    exe->constant_pool
        = (DVM_ConstantPool *)MEM_realloc(exe->constant_pool,
...
copy_type_specifier_no_alloc(TypeSpecifier *src, DVM_TypeSpecifier *dest)
{
    ...
    dest->derive = (DVM_TypeDerive*)MEM_malloc(sizeof(DVM_TypeDerive) * derive_count);
...
dkc_copy_type_specifier(TypeSpecifier *src)
{
    ...
    dest = (DVM_TypeSpecifier*)MEM_malloc(sizeof(DVM_TypeSpecifier));
...
add_class(DVM_Executable *exe, ClassDefinition *cd, DVM_Class *dest)
{
    ...
    if (cd->super_class) {
        dest->super_class =(DVM_ClassIdentifier *) MEM_malloc(sizeof(DVM_ClassIdentifier));
    ...
    dest->interface_ =(DVM_ClassIdentifier*) MEM_malloc(sizeof(DVM_ClassIdentifier)
                                  * interface_count);
    interface_count = 0;
    for (if_pos = cd->interface_list; if_pos; if_pos = if_pos->next) {
    ...
    dest->field =(DVM_Field*) MEM_malloc(sizeof(DVM_Field) * field_count);
    dest->method_count = method_count;
    dest->method = (DVM_Method*)MEM_malloc(sizeof(DVM_Method) * method_count);
...
fix_opcode_buf(OpcodeBuf *ob)
{
    ...
    fix_labels(ob);
    ret = (DVM_Byte*)MEM_realloc(ob->code, ob->size);
...
add_line_number(OpcodeBuf *ob, int line_number, int start_pc)
{
    if (ob->line_number == NULL
        || (ob->line_number[ob->line_number_size-1].line_number
            != line_number)) {
        ob->line_number = (DVM_LineNumber*)MEM_realloc(ob->line_number,
...
static void
generate_code(OpcodeBuf *ob, int line_number, long code,  ...)
{
    va_list     ap;
    int         i;
    char        *param;
    ...
    if (ob->alloc_size < ob->size + 1 + (param_count * 2)) {
        ob->code =(DVM_Byte*) MEM_realloc(ob->code, ob->alloc_size + OPCODE_ALLOC_SIZE);
...
int
get_opcode_type_offset2(TypeSpecifier *type)
{
    if (type->derive != NULL) {
    ...
    for (cd_pos = cd; cd_pos; cd_pos = cd_pos->super_class) {
        for (member_pos = cd_pos->member; member_pos;
            ...
            if (member_pos->u.field.initializer) {
				DBG_panic(("Field initializer not supported!"));
				/*
...
add_classes(DKC_Compiler *compiler, DVM_Executable *exe)
{
    ...
    for (cd_pos = compiler->class_definition_list; cd_pos;
        ...
        BcGenerateFieldInitializer(exe, cd_pos, dvm_class);
    }
...
int add_type_specifier(TypeSpecifier *src, DVM_Executable *exe)
{
    int ret;

    exe->type_specifier
        = (DVM_TypeSpecifier*)MEM_realloc(exe->type_specifier,
                       ...
                       Expression *expr, OpcodeBuf *ob)
{
    if (expr->kind == IDENTIFIER_EXPRESSION) {
        ...
        generate_expression(exe, block, expr->u.index_expression.barray, ob);
...
get_label(OpcodeBuf *ob)
{
    ...
    if (ob->label_table_alloc_size < ob->label_table_size + 1) {
        ob->label_table = (LabelTable*)MEM_realloc(ob->label_table,
...
int get_method_index_Ex(MemberExpression *member,int* outParamCnt)
{
    int method_index;
    ...
    if (dkc_is_array(member->expression->type)
		...
		if(outParamCnt)
			*outParamCnt=member->declaration->u.method.function_definition->param_cnt;
    }

    return method_index;
}
int get_method_index(MemberExpression *member)
{
	return get_method_index_Ex(member,0);
}
...
static void
add_try_to_opcode_buf(OpcodeBuf *ob, DVM_Try *blktry)
{
    ob->blktry =(DVM_Try*) MEM_realloc(ob->blktry, sizeof(DVM_Try) * (ob->try_size+1));
    ob->blktry[ob->try_size] = *blktry;
    ob->try_size++;
}
                       ...
                       Statement *statement, OpcodeBuf *ob)
{
    ...
    dvm_catch = (DVM_CatchClause*) MEM_malloc(sizeof(DVM_CatchClause) * catch_count);
             ...
             DVM_Boolean in_this_exe,ClassDefinition* cdef)
{
	...
	char* clsname=0;
    if (src->block && in_this_exe) { //modiefied
        //init_opcode_buf(&ob);
		//printf("FUN: %s @ %s\n",src->name ,exe->package_name );
		if(cdef)
			clsname=cdef->name;
        Function* fun=BcGenerateFunction(exe, src,clsname);
		//dest->pllvmfun=fun;
        dest->is_implemented = DVM_TRUE;
		dest->code_block.code_size = 0;
        dest->code_block.code = 0;
        dest->code_block.line_number_size = 0;
        dest->code_block.line_number = 0;
        dest->code_block.line_number = 0;
        dest->code_block.try_size = 0;
        dest->code_block.blktry =0;
        dest->code_block.code=0;
        dest->code_block.code_size=0;
        ...
        dest->code_block.blktry = ob.blktry;
...
add_functions(DKC_Compiler *compiler, DVM_Executable *exe)
{
	...
	//printf("ADD FUN PATH:%s\n",compiler->path);
    in_this_exe =(DVM_Boolean*) MEM_malloc(sizeof(DVM_Boolean)
                                     * compiler->dvm_function_count);
    for (i = 0; i < compiler->dvm_function_count; i++) {
        in_this_exe[i] = DVM_FALSE;
    }
    for (fd = compiler->function_list; fd; fd = fd->next) {
		//printf("@@@@@@@@@@@@@@@@@@@@@@@@@@\n%d   %d\n",fd->class_definition , fd->block);
        if (fd->class_definition && fd->block == NULL) // modiefied fix-me : &&
            continue;

        dest_idx = search_function(compiler, fd);
        in_this_exe[dest_idx] = DVM_TRUE;
        add_function(exe, fd, &compiler->dvm_function[dest_idx], DVM_TRUE,fd->class_definition );
    }
    ...
    for (i = 0; i < compiler->dvm_function_count; i++) {
        ...
        add_function(exe, fd, &compiler->dvm_function[i], DVM_FALSE,fd->class_definition);
    }
...
add_top_level(DKC_Compiler *compiler, DVM_Executable *exe)
{
    ...
    //exe->top_level.blktry = ob.blktry;
...
generate_constant_initializer(DKC_Compiler *compiler, DVM_Executable *exe)
{
    ...
    exe->constant_initializer.blktry = ob.blktry;
