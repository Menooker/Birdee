...
dkc_create_type_specifier(DVM_BasicType basic_type)
{
	...
	if(basic_type==DVM_DELEGATE_TYPE)
		type->u.delegate_ref.delegate_definition =0;
    return type;
}
...
Expression *
dkc_create_assign_expression(Expression *left, AssignmentOperator boperator,
                             Expression *operand)
{
    Expression *exp;
    ...
    exp->u.assign_expression.boperator = boperator;
...
Expression *
dkc_create_binary_expression(ExpressionKind boperator,
                             Expression *left, Expression *right)
{
    Expression *exp;
    exp = dkc_alloc_expression(boperator);
...
Expression *
dkc_create_index_expression(Expression *barray, Expression *index)
{
    Expression *exp;

    exp = dkc_alloc_expression(INDEX_EXPRESSION);
    exp->u.index_expression.barray = barray;
...
Expression *
BcCreateAutoExpression(char *identifier)
{
    Expression  *exp;

    exp = dkc_alloc_expression(AUTOVAR_EXPRESSION);
    exp->u.identifier.name = identifier;
